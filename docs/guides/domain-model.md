...
| `CommentAdded` | `{ commentId, body, authorId, attachments?, createdAt }` |
...
Comment エンティティのテーブル。指摘に対するコメントと添付写真を管理する。
...
| `author_id` | uuid | NOT NULL | 投稿者の User ID |
| `version` | integer | NOT NULL | コメント作成時の Issue イベント version |
| `created_at` | timestamptz | NOT NULL | 作成日時 |
