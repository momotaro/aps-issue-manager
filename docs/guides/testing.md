...
- **`vi.useFakeTimers()` は使用禁止**: fake timers は `setTimeout` / `setInterval` を偽装するため、DB ドライバ（postgres.js）やMinIO クライアント（@aws-sdk/client-s3）の内部タイマーがハングし、テストがタイムアウトする。時刻差が必要なテストでは `setTimeout` による sleep で対応する
