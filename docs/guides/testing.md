...
| アップロードフロー | Presigned URL 取得 → MinIO 直接 PUT → attachments リストへの追加。confirm API はフロントから呼ばない（backend の useCase 内で pending → confirmed 移動） |
| 写真削除 | pending / confirmed の両状態からの削除 |
| URL 構築 | `getPhotoUrl(storagePath)` による MinIO URL 構築 |
| 日時整形 | `formatDateTime()` のフォーマット結果 |
| Timeline 表示 | コメント・ステータス変更イベントのレンダリング、写真サムネイルからの lightbox 起動 |
